...
### Если `vite` не найден на Windows
Если при `npm run build` появляется ошибка вида `"vite" не является внутренней или внешней командой`, обычно не установлены локальные зависимости проекта. Выполните:
...
Если проблема остается, удалите `node_modules` и `package-lock.json`, затем повторите установку:

Удалите папку `node_modules` и файл `package-lock.json` вручную, затем выполните:
